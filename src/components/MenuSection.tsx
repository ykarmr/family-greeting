...
export function MenuSection() {
  ...
  return (
    <section
      ref={ref}
      className="px-6 md:px-12 lg:px-20 py-24 bg-[var(--color-beige-light)] relative overflow-hidden"
    >
      {/* 背景テクスチャ */}
      <div
          ...
          <div className="border-[3px] border-double border-[var(--color-coffee)]/20 h-full p-6 md:p-10 relative">
            ...
            <div className="text-center mb-12">
              <div className="inline-block border-b-2 border-[var(--color-text-dark)] pb-2 mb-2">
                <span className="font-serif font-bold text-3xl tracking-[0.2em] text-[var(--color-text-dark)]">
                  本日のメニュー
                </span>
              </div>
              <p className="font-serif text-base text-[var(--color-text-light)] tracking-widest">
                {menu.course}
              </p>
            </div>
            ...
            <div className="space-y-6">
              {menu.items.map((item) => (
                <div key={item} className="flex items-baseline gap-4 group">
                  ...
                  <span className="text-[10px] text-[var(--color-text-light)] tracking-widest shrink-0 font-serif opacity-0 group-hover:opacity-100 transition-opacity">
                    SPECIAL
                  </span>
                </div>
              ))}
            ...
            <div className="mt-16 pt-8 border-t border-[var(--color-text-light)]/20 text-center">
              <span className="inline-block bg-[var(--color-text-dark)] text-white text-xs px-4 py-1 tracking-widest mb-4 font-serif rounded-sm">
                お飲み物
              </span>

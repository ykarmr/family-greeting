...
function ScrapbookCard({
  ...
  return (
    <div
      className={`relative p-8 md:p-12 transition-all duration-1000 ${isInView ? "opacity-100 rotate-0" : "opacity-0 rotate-1"}`}
    >
      {/* 背景の紙 */}
            ...
            >
                ...
                {member.image ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <span className="text-3xl opacity-50 filter grayscale">
                      ...
                      {!["父", "母"].includes(member.relation) &&
                        !member.relation.match(/(兄|弟|姉|妹)/) &&
                        "👤"}
                    </span>
                  </div>
                )}
              ...
              <div className="flex-1 pt-2">
                <div className="flex items-baseline gap-3 mb-1">
                  <span
                    className="font-serif text-[var(--color-text-dark)] text-xl border-b border-[var(--color-text-light)]/50 pb-0.5"
                ...
                <p className="text-xs text-[var(--color-text-light)] mb-2 tracking-wider">
                  {member.furigana}
                </p>
                <p className="font-handwritten text-[var(--color-text)] text-base leading-7 relative">
                  ...
                  ></span>
                  <span className="relative z-10 block pt-[4px]">
                    {member.description}
                  </span>
                </p>
              </div>
...
export function GroomFamilySection() {
  ...
  return (
    <section
      ref={ref}
      className="px-6 md:px-12 lg:px-20 py-24 bg-[var(--color-cream)] overflow-hidden"
    >
...
export function BrideFamilySection() {
  ...
  return (
    <section
      ref={ref}
      className="px-6 md:px-12 lg:px-20 py-24 bg-[var(--color-cream)] overflow-hidden relative"
    >
      {/* 連結用の装飾ライン */}
